...
from pyomo.common.autoslots import AutoSlots, StateBrowser
from pyomo.common.collections import Mapping
from pyomo.common.deprecation import deprecated, deprecation_warning, RenamedClass
from pyomo.common.errors import TemplateError
...
class BlockData(ActiveComponentData):
    ...
    def __delattr__(self, name):
        ...
        if name in self._decl:
            ...
            #
            # Other Python objects are removed with the standard __delattr__
            # method.
            #
    ...
    def construct(self, data=None):
        ...
        try:
            if not self.is_indexed():
                ...
                return
            elif TEMPLATIZE_BLOCKS:
                self.__class__ = IndexedBlockTemplate
                self._storage = MutableBlockDataTemplateStorage(self)
                try:
                    # Note: we need to wrap the block rule, so that we
                    # can intercept the IndexTemplate generated by
                    # templatize_rule and store it as the
                    # BlockDataTemplateStorage._index.
                    _block, self._indices = templatize_rule(
                        self._storage, _rule_wrapper(self._rule), self.index_set()
                    )
                    if _block is None or _block is self._storage:
                        # Successful templatization; we are done.
                        self._data = {
                            idx: BlockDataView(self, idx) for idx in self.index_set()
                        }
                        return
                    del self._indices
                except TemplateError:
                    # We can get here if the block rule evaluated the
                    # index (raising a TemplateExpressionError), or
                    # someone attempts to modify the block storage
                    # object outside of the rule (raising a
                    # TemplateError), or a rule returned a
                    # non-IndexedComponent component (raising a
                    # TemplateError)
                    pass
                finally:
                    self._storage.__class__ = BlockDataTemplateStorage
                # Unsuccessful templatization; undo _storage (and
                # _indices) and return to normal processing.
                del self._storage
...
class _rule_wrapper:
    ...
    def __call__(self, block, indices):
        block._index = indices
        if normalize_index.flatten:
            block._index = normalize_index(block._index)
        return self._rule(block, indices)


class _DeclOrderView(MutableSequence):
    def __init__(self, block_view):
        self._ref = block_view.parent_component()._storage._decl_order
    ...
    def __getitem__(self, item):
        obj, ptr = self._ref[item]
        ref = Reference(obj[self._view._index, ...])
        ref._name = obj._name
        return (ref, ptr)
...
class BlockDataView(BlockData):
    def __init__(self, component, index):
        super(BlockData, self).__init__(component)
        # Remove the local block storage.  We will regnerate it only if
        # someone adds a component.
        # super(BlockData, self).__setattr__('_ctypes', None)
        # super(BlockData, self).__setattr__('_decl', None)
        super(BlockData, self).__setattr__('_decl_order', _DeclOrderView(self))
        super(BlockData, self).__setattr__('_private_data', None)
        super(BlockData, self).__setattr__('_index', index)

    def __getattr__(self, attr):
        if '_decl' in self.__dict__:
            # This View has local specializations, so the local storage
            # is definitive.
            return super().__getattr__(attr)
        # We are going to pass the lookup to the central template
        # storage.  Components need to be mapped to References.
        _storage = self.parent_component()._storage
        if attr in _storage._decl:
            return Reference(getattr(_storage, attr)[self._index, ...])
        else:
            return getattr(_storage, attr)

    def _compact_decl_storage(self):
        if '_decl' in self.__dict__:
            # Only call the method on the local object if it is the
            # definitive data store
            super()._compact_decl_storage()
        self.parent_component()._storage._compact_decl_storage()

    def __delattr__(self, attr):
        # We disallow deleting anything from the template storage object
        if attr in self.parent_component()._storage._decl:
            raise TemplateError(
                "Cannot delete component from one index in a templatized block"
            )
        return super().__delattr__(attr)

    def __setattr__(self, attr, val):
        # We disallow changing anything from the template storage object
        if attr == '_component':
            return super().__setattr__(attr, val)
        if attr in self.parent_component()._storage._decl:
            raise TemplateError(
                "Cannot modify component on one index in a templatized block"
            )
        return super().__setattr__(attr, val)


class BlockDataTemplateStorage(BlockData):
    def index(self):
        # Override BlockData.index() to bypass check that _index is in _data
        return self._index

    def __setattr__(self, attr, val):
        raise TemplateError("Attempt to modify immutable block template storage")

    def __delattr__(self, attr):
        raise TemplateError(
            "Attempt to delete component from immutable block template storage"
        )

    def getname(self, fully_qualified=False, name_buffer=None, relative_to=None):
        # This causes the fully-qualified name for components stored
        # here to be slightly different.  Assuming this is block 'b' and
        # there is a component 'x', then the x's data wwould look like:
        # 'b[*].x[_1]'.
        #
        # TODO: is '[*]' the best nomenclature?  Maybe something without
        # the indexing notation ('b*.x[_1]'?) or from unicode (dagger?)?
        return (
            self.parent_component().getname(fully_qualified, name_buffer, relative_to)
            + '[*]'
        )


class MutableBlockDataTemplateStorage(BlockDataTemplateStorage):

    def __getattribute__(self, name):
        _ga = super().__getattribute__
        try:
            if name in _ga('_decl'):
                index = _ga('_index')
                if index.__class__ is tuple:
                    return GetItemExpression((_ga(name), *index))
                else:
                    return GetItemExpression((_ga(name), index))
        except AttributeError:
            pass
    ...
    def __delattr__(self, attr):
        return super(BlockDataTemplateStorage, self).__delattr__(attr)

    def __setattr__(self, name, val):
        if not isinstance(val, Component):
            return super(BlockDataTemplateStorage, self).__setattr__(name, val)
        if val._constructed:
            raise TemplateError(
                f"Cannot templatize block {self.name}: new component {name} "
                "was constructed before assigning to the block"
            )
        # This is a new component.  We need to add the template indices
        # to this component's index set.
        block_index_set = self.parent_component().index_set()
        if val.is_indexed():
            # Already indexed: just add this index to the indexing set
            val._index_set = block_index_set * val.index_set()
            state = StateBrowser(val)
        else:
            # Scalar: we need to convert it into an indexed component
            if not isinstance(val, IndexedComponent):
                raise TemplateError(
                    f"Error adding template index to {name}: "
                    f"({val.__class__.__name__} is not an IndexedComponent"
                )
            # Find the Component
            cls = [
                c
                for c in val.__class__.__mro__
                if issubclass(c, IndexedComponent) and not issubclass(c, ComponentData)
            ][0]
            # Create a new indexed component using the block's
            # index_set.  Then, copy over all the attribured (except the
            # _index_set) from the original scalar that the original
            # block rule created.
            new_val = cls(block_index_set)
            state = StateBrowser(new_val)
            for attr, dat in StateBrowser(val).items():
                if attr in state and attr != '_index_set':
                    state[attr] = dat
            val = new_val

        # We need to update all the rules on the new component to ignore
        # the (block) template indices we just added to the component:
        for attr, dat in state.items():
            if isinstance(dat, InitializerBase):
                state[attr] = BlockTemplateInitializer(dat, block_index_set)
        state.setstate(val)
        # Finally, resume normal processing to add the component to this
        # storage block
        with _TemplateIterManager.pause(not isinstance(val, Block)):
            return super(BlockDataTemplateStorage, self).__setattr__(name, val)


class IndexedBlockTemplate(IndexedBlock):
    pass
