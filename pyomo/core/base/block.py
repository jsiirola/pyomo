    ...
    def construct(self, data=None):
        ...
        try:
            if not self.is_indexed():
                ...
                self._storage = BlockDataTemplateStorage(self)
                try:
...
class BlockDataTemplateStorage(BlockData):
    ...
    def __getattribute__(self, name):
        ...
        try:
            comp_idx = _ga(self, '_decl').get(name, None)
        except AttributeError:
            return _ga(self, name)
        if comp_idx is not None:
            comp = _ga(self, '_decl_order')[comp_idx][0]
            index = _ga(self, '_index')
            if index.__class__ is tuple:
                return GetItemExpression((comp, *index))
            else:
                return GetItemExpression((comp, index))
        else:
            return _ga(self, name)
        ...
        if name in self._decl:
            return GetItemExpression((getattr(self._storage, name), *self._index))
            # return Reference(getattr(self._storage, name)[self._index, ...])
        else:
    ...
    def __setattr__(self, name, val):
        ...
        if val.is_indexed():
            # just add this index to the indexing set
        ...
        # We need to update all the rules to ignore the (block) template indices
