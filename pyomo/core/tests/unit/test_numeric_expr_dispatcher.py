...
class TestExpressionGeneration_Misc(Base, unittest.TestCase):
    ...
    def test_categorize_arg_type(self):
        ...
        NED = NumericExpressionDispatcher

        saved_known_arg_types = dict(NED.known_arg_types)
        saved_native_numeric_types = set(native_numeric_types)
        saved_native_types = set(native_types)
        try:
            NED.known_arg_types.clear()
            native_types.discard(int)
            native_numeric_types.discard(int)
            self.assertIs(NED.categorize_arg_type(10), ARG_TYPE.NATIVE)
            self.assertIn(int, NED.known_arg_types)
            ...
            self.assertIs(NED.categorize_arg_type(self.invalid), ARG_TYPE.INVALID)
            self.assertIs(NED.categorize_arg_type(self.asbinary), ARG_TYPE.ASNUMERIC)
            self.assertIs(NED.categorize_arg_type(self.zero), ARG_TYPE.NATIVE)
            self.assertIs(NED.categorize_arg_type(self.one), ARG_TYPE.NATIVE)
            self.assertIs(NED.categorize_arg_type(self.native), ARG_TYPE.NATIVE)
            self.assertIs(NED.categorize_arg_type(self.npv), ARG_TYPE.NPV)
            self.assertIs(NED.categorize_arg_type(self.param), ARG_TYPE.PARAM)
            self.assertIs(NED.categorize_arg_type(self.param0), ARG_TYPE.PARAM)
            self.assertIs(NED.categorize_arg_type(self.param1), ARG_TYPE.PARAM)
            self.assertIs(NED.categorize_arg_type(self.param_mut), ARG_TYPE.PARAM)
            self.assertIs(NED.categorize_arg_type(self.var), ARG_TYPE.VAR)
            self.assertIs(NED.categorize_arg_type(self.mon_native), ARG_TYPE.MONOMIAL)
            self.assertIs(NED.categorize_arg_type(self.mon_param), ARG_TYPE.MONOMIAL)
            self.assertIs(NED.categorize_arg_type(self.mon_npv), ARG_TYPE.MONOMIAL)
            self.assertIs(NED.categorize_arg_type(self.linear), ARG_TYPE.LINEAR)
            self.assertIs(NED.categorize_arg_type(self.sum), ARG_TYPE.SUM)
            self.assertIs(NED.categorize_arg_type(self.other), ARG_TYPE.OTHER)
            self.assertIs(NED.categorize_arg_type(self.mutable_l0), ARG_TYPE.MUTABLE)
            self.assertIs(NED.categorize_arg_type(self.mutable_l1), ARG_TYPE.MUTABLE)
            self.assertIs(NED.categorize_arg_type(self.mutable_l2), ARG_TYPE.MUTABLE)
            self.assertIs(NED.categorize_arg_type(self.mutable_l3), ARG_TYPE.MUTABLE)

            # Other interesting tests (partially for coverage)
            self.assertIs(NED.categorize_arg_type(self.m), ARG_TYPE.INVALID)
            self.assertIs(
                NED.categorize_arg_type(CustomAsNumeric(10)), ARG_TYPE.ASNUMERIC
            )
            self.assertIs(NED.categorize_arg_type(CustomNumeric()), ARG_TYPE.OTHER)

        finally:
            NED.known_arg_types.clear()
            NED.known_arg_types.update(saved_known_arg_types)
