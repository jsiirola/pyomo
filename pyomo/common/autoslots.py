...
class StateBrowser:
    """Present a dict-like interface to the state generated by AutoSlots

    A class that builds on AutoSlots has a state that is designed for
    efficiency and not interrogation.  Notably, all slots are stored in
    an unlabeled list, so determining the attributes that correspond to
    entries is impossible without knowing the internal structure of the
    `__auto_slots__` class attribute.  This class takes a `class` and
    `state` and presents a (writable) dict-like interface to the state.

    Parameters
    ----------
    cls_or_obj : type | object

        The class or the object to browse.  The type (or object) must
        correspond to a class (or instance of a class) derived from
        either the :class:`AutoSlots` meta class or the
        :class:`AutoSlots.Mixin` class.

    state : list | None

        The state to browse.  If `state` is None, then `cls_or_obj` must
        be an instance (and we will immediately collect the state)

    """

    def __init__(self, cls_or_obj, state=None):
        self._autoslot_info = cls_or_obj.__auto_slots__
        if state is None:
            self._state = cls_or_obj.__getstate__()
        else:
            self._state = state
    ...
    def __getitem__(self, item):
        if self._autoslot_info.has_dict and item in self._state[-1]:
            return self._state[-1][item]
        if self._slot_mapper is None:
            self._slot_mapper = {k: i for i, k in enumerate(self._autoslot_info.slots)}
        return self._state[self._slot_mapper[item]]

    def __setitem__(self, item, value):
        if self._autoslot_info.has_dict and item in self._state[-1]:
            self._state[-1][item] = value
            return
        if self._slot_mapper is None:
            self._slot_mapper = {k: i for i, k in enumerate(self._autoslot_info.slots)}
        if item not in self._slot_mapper:
            raise KeyError(item)
        self._state[self._slot_mapper[item]] = value
    ...
    def __contains__(self, item):
        ...
        if self._slot_mapper is None:
            self._slot_mapper = {k: i for i, k in enumerate(self._autoslot_info.slots)}
        return item in self._slot_mapper

    def __len__(self):
        if self._autoslot_info.has_dict:
            return len(self._autoslot_info.slots) + len(self._state[-1])
        else:
            return len(self._autoslot_info.slots)

    def keys(self):
        if self._autoslot_info.has_dict:
            return chain(self._autoslot_info.slots, self._state[-1].keys())
        else:
            return iter(self._autoslot_info.slots)

    def values(self):
        if self._autoslot_info.has_dict:
            return chain(self._state[:-1], self._state[-1].values())
        else:
            return iter(self._state)

    def items(self):
        if self._autoslot_info.has_dict:
            # Note: 'strict' isn't required, but is a reminder that slots
            # is 1 element shorter than state in this situation
            return chain(
                zip(self._autoslot_info.slots, self._state, strict=False),
                self._state[-1].items(),
            )
        else:
            return zip(self._autoslot_info.slots, self._state)

    def setstate(self, obj):
        obj.__setstate__(self._state)
